...
print('nn = '+str(nn)+', gg = '+str(gg)+', ss = '+str(ss), flush=True)
...
for pp in range(0,nperiods,1):
	print(reg_groups[gg] + ' | ' + seasons[ss]+ ' | ' + period_str[pp], flush=True)
	event_subset_df = all_events_3d[(all_events_3d['region']==reg_groups[gg])&(all_events_3d['season']==seasons[ss])&(all_events_3d['period']==period_str[pp])].reset_index()
	...
	events_char_pp_xx_xr.to_netcdf(dir_comps+'CESM2-LE_'+ens_string+'_p'+pxx_string+'_FWI_events_char_composites_'+period_str[pp]+'_moving_'+reg_groups[gg]+'_'+seasons[ss]+'.nc')
	print('\n')
