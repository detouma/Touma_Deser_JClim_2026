...
files_list = []
for file in sorted(glob.glob(dir_event_3d+'CESM2-LE_*_mean_VP_mapped_'+var_name+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_NWHemi.nc')):
...
file_event_3d = dir_event_3d+'CESM2-LE_'+ens_no_string[ens]+'_mean_VP_mapped_'+var_name+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_NWHemi.nc'
...
file_event_2d = dir_event_2d+'CESM2-LE_'+ens_no_string[ens]+'_mean_VP_mapped_'+var_name+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_daily_2d_event_numbers_'+str(year0)+'-'+str(year1)+'_NWHemi.nc'
...
xr_event.attrs['description'] = 'Extreme fire weather events for '+reg_groups[gg]+' using historically mean mapped '+var_name+' exceeding the historic p'+str(pxx)+' threshold'
print('writing xarray to netcdf' , flush=True)
xr_event.to_netcdf(dir_event+'CESM2-LE_'+ens_no_string[ens]+'_mean_VP_mapped_'+var_name+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_'+reg_groups[gg]+'.nc')
