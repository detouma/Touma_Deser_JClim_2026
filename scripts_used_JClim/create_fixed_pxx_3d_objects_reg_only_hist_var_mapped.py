...
dir_reg_only = '/glade/derecho/scratch/detouma/extreme_objects/fwi_objects/reg_only/dist_mapped/'
...
xr_event.attrs['description'] = 'Extreme fire weather events for '+WUS_regions[rr]+' using historically mapped '+var_name+' exceeding the historic p'+str(pxx)+' threshold'
print('writing xarray to netcdf' , flush=True)
xr_event.to_netcdf(dir_reg_only+'CESM2-LE_'+ens_no_string[ens]+'_mapped_'+var_name+'_to_'+str(n_ens_dist)+'_ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_'+WUS_regions[rr]+'.nc')
