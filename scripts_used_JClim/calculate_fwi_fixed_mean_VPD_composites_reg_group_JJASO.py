...
vps = xr_vp.VPs.sel(time=xr_vp.time.dt.year.isin(range(year0,year1+1,1)))
vpd = xr.DataArray(vps - vp, coords = vp.coords, name = 'VPD')
...
print('calculate vpd mov anomaly', flush=True)
vpd_mov_mean_files_list = []

for yy in years:
    vpd_mov_mean_files_list.append(dir_vp+'mean/CESM2-LE_allens_mean_30day_mov_34year_mov_avg_VPD_'+str(year0)+'_NWHemi.nc')

vpd_mov_mean_xr = xr.open_mfdataset(vpd_mov_mean_files_list, preprocess=preprocess)
vpd_mov_mean_xr['year'] =  years
vpd_mov_mean_doy = vpd_mov_mean_xr.VPD
vpd_mov_mean = np.reshape(np.array(vpd_mov_mean_doy),newshape=((nyears*365),len(vpd_mov_mean_doy.lat),len(vpd_mov_mean_doy.lon)))
vpd_mov_anom = vpd - vpd_mov_mean

#calculate vp anomaly
print('calculate vpd anomaly', flush=True)
f_vpd_mean = dir_vp+'mean/CESM2-LE_allens_mean_30day_mov_34year_mov_avg_VPD_'+str(year0)+'_NWHemi.nc'
xr_vpd_mean = xr.open_dataset(f_vpd_mean)
vpd_mean_365 = np.array(xr_vpd_mean.VPD) 
vpd_mean = np.tile(vpd_mean_365,[nyears,1,1])
vpd_anom = vpd-vpd_mean
...
for pp in range(0,nperiods,1):
	...
	print('vpd_anom', flush=True) 
	events_char_pp_xx[3,:,:] = np.ma.mean(np.ma.masked_array(vpd_anom,mask=(pp_xx_event_mask==0)),axis=0)
	print('vpd_mov_anom', flush=True) 
	events_char_pp_xx[4,:,:] = np.ma.mean(np.ma.masked_array(vpd_mov_anom,mask=(pp_xx_event_mask==0)),axis=0)
	...
	events_char_pp_xx_xr.to_netcdf(dir_comps+'CESM2-LE_'+ens_string+'_p'+pxx_string+'_FWI_events_char_VPD_composites_'+period_str[pp]+'_fixed_'+reg_groups[gg]+'_'+season+'.nc')
	print('\n')
