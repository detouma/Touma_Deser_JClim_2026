...
xr_event.attrs['description'] = 'Extreme fire weather events using historically mean VP mapping for '+var_name+' exceeding the historic p'+str(pxx)+' threshold'
print('writing xarray to netcdf' , flush=True)
xr_event.to_netcdf(dir_event+'CESM2-LE_'+ens_string[nn]+'_mean_VP_mapped_'+var_name+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_NWHemi.nc')
