...
vv = int(sys.argv[2])
ens_subset_len0 = int(sys.argv[3]) # how many ensemble members to use (tested with 20)
...
fdate_inds = np.where((fyyyy0<=(year1+int(nyears_window/2)))&(fyyyy1>=(year0-int(nyears_window/2))))[0]
files_window = list(files_list[i] for i in list(fdate_inds))

ens_window = list(ens_string[i] for i in list(fdate_inds))
ens_unique = sorted(list(set(ens_window)))

ens_inds_0 = np.where(np.array(ens_window)==ens_unique[0])[0]
nfiles_window = len(ens_inds_0)
xr_ens0 = xr.open_dataset(dir_var+files_window[0])
lat0 = xr_ens0.lat
lon0 = xr_ens0.lon
...
ens_subset_len = np.min([len(ens_unique),ens_subset_len0])
...
for nn in range(0,ens_subset_len,1):
	...
	ens_nn = ens_unique[nn]
	print(nn, end=': ', flush=True)
	ens_inds = np.where(np.array(ens_window)==ens_nn)[0]
	files_ens  = list(files_window[i] for i in list(ens_inds))
	tt_doy_count = 0
	tt_hist_count = 0
	for ff in files_ens:
		xr_ens_ff = xr.open_dataset(dir_var+ff)
		nc_ens_ff = Dataset(dir_var+ff, 'r')
		ens_ff_time = xr_ens_ff.time
		#####
		ens_ff_var_doy_inds = np.where((np.isin(np.array(ens_ff_time.dt.year),range(year0-int(nyears_window/2),year1+int(nyears_window/2)+1,1)))
								   &(np.isin(np.array(ens_ff_time.dt.dayofyear),doy_range))
								   &(~ens_ff_time.get_index("time").duplicated())
		)[0]
		if len(ens_ff_var_doy_inds>0):
			var_doy[nn,tt_doy_count:tt_doy_count+len(ens_ff_var_doy_inds),:,:] = nc_ens_ff[var_name][ens_ff_var_doy_inds,lat_inds,lon_inds]
			tt_doy_count += len(ens_ff_var_doy_inds)
			if nn==0:
				var_doy_time_list.append(ens_ff_time[ens_ff_var_doy_inds])
		#####
		ens_ff_hist_doy_inds = np.where((np.isin(np.array(ens_ff_time.dt.year),pxx_years))
								   &(np.isin(np.array(ens_ff_time.dt.dayofyear),doy_range))
								   &(~ens_ff_time.get_index("time").duplicated())
		)[0]
		if len(ens_ff_hist_doy_inds>0):
			hist_doy[nn,tt_hist_count:tt_hist_count+len(ens_ff_hist_doy_inds),:,:] = nc_ens_ff[var_name][ens_ff_hist_doy_inds,lat_inds,lon_inds]
			tt_hist_count += len(ens_ff_hist_doy_inds)
			if nn==0:
				hist_doy_time_list.append(ens_ff_time[ens_ff_hist_doy_inds])
		#####
		ens_ff_mid_doy_inds = np.where((np.isin(np.array(ens_ff_time.dt.year),range(year0,year1+1,1)))
								   &(np.array(ens_ff_time.dt.dayofyear)==doy)
								   &(~ens_ff_time.get_index("time").duplicated())
		)[0]
		if len(ens_ff_mid_doy_inds>0):
			ens_ff_mid_doy_inds_years = ens_ff_time.dt.year[ens_ff_mid_doy_inds]
			var_mid_doy_year_inds = np.where(np.isin(np.arange(year0,year1+1,1),ens_ff_mid_doy_inds_years))[0]	
			var_mid_doy[nn,var_mid_doy_year_inds,:,:] = nc_ens_ff[var_name][ens_ff_mid_doy_inds,lat_inds,lon_inds]
			if nn==0:
				var_mid_doy_time_list.append(ens_ff_time[ens_ff_mid_doy_inds]) 
...
for yy in range(0,nyears,1):
	...
	for ii in range(0,nlat,1):
		for jj in range(0,nlon,1):
			...
			# ind_ii_jj_yy_left = searchsorted_left(var_doy_year_all_sorted[:,ii,jj],var_mid_doy[:,yy,ii,jj])
			# ind_ii_jj_yy_right = searchsorted_right(var_doy_year_all_sorted[:,ii,jj],var_mid_doy[:,yy,ii,jj])
...
var_mapped_ens_doy_xr = xr.DataArray(var_mapped_ens_doy, name=var_name, dims=["ens_member", "time", "lat", "lon"], coords=dict(ens_member=ens_unique[:ens_subset_len], time=var_mid_doy_time, lat=lat, lon=lon,), attrs=dict(description=var_name+ ' mapped to '+str(pxx_year)+' distribution', units=xr_ens0_units, members_used=str(ens_subset_len)),)
