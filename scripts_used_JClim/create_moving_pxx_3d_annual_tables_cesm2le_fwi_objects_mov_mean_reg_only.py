...
dir_event = '/glade/campaign/cgd/cas/detouma/fwi_objects/3d_moving_objects/'
...
event_files_list = []
for file in sorted(glob.glob(dir_event+'CESM2-LE_*_allens_moving_p'+str(pxx)+'_FWI_3d_event_numbers_'+str(year0)+'-'+str(year1)+'_'+WUS_regions[rr]+'_only.nc')):
...
file_event = dir_event+'CESM2-LE_'+ens_no_string[ens]+'_allens_moving_p'+str(pxx)+'_FWI_3d_event_numbers_'+str(year0)+'-'+str(year1)+'_'+WUS_regions[rr]+'_only.nc'
...
# output event file
event_3d_df_nonan.to_csv(dir_tables+'CESM2-LE_'+ens_no_string[ens]+'_FWI_allens_moving_p'+str(pxx)+'_3d_events_'+str(year0)+'-'+str(year1)+'_'+WUS_regions[rr]+'.txt', header=True, index=None, sep=' ', mode='w', float_format = "%.2f" )
