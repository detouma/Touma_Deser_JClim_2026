...
files_list = []
for file in sorted(glob.glob(dir_event_3d+'CESM2-LE_*_allens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_U10_NWHemi.nc')):
...
file_event_3d = dir_event_3d+'CESM2-LE_'+ens_no_string[ens]+'_allens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_U10_NWHemi.nc'
...
file_event_2d = dir_event_2d+'CESM2-LE_'+ens_no_string[ens]+'_allens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_daily_2d_event_numbers_'+str(year0)+'-'+str(year1)+'_U10_NWHemi.nc'
...
print('nevents = '+str(n_events_3d_reg))
new_event_3d_no = 1
...
file_event = dir_event_3d+'CESM2-LE_'+ens_no_string[ens]+'_allens_fixed_p'+str(pxx)+'_FWI_3d_event_numbers_'+str(year0)+'-'+str(year1)+'_'+WUS_regions[rr]+'_only_U10.nc'
nc_event = Dataset(file_event,'w',format='NETCDF4')
nc_event.description = 'All ensembles (1-100) p'+str(pxx)+' Fire Weather Index exceedence event numbers for region. SciPy functions used: label().'
