...
files_list = [] 
for file in sorted(glob.glob(dir_fwi+'FWI_CESM2-LE_*_historic_'+str(pxx_year)+'_distribution_mapped_'+var_name+'*.nc')):
...
xr_event.attrs['description'] = 'Extreme fire weather events using historically mapped '+var_name+' exceeding the historic p'+str(pxx)+' threshold'
print('writing xarray to netcdf' , flush=True)
xr_event.to_netcdf(dir_event+'CESM2-LE_'+ens_string[nn]+'_mapped_'+var_name+'_to_'+str(n_ens_dist)+'_ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_daily_2d_event_numbers_'+str(year0)+'-'+str(year1)+'_NWHemi.nc')
