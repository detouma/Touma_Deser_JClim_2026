...
nlat = len(lat)
nlon = len(lon)
fwi_pxx_2d = nc_pxx.variables['FWI'][xx,lat_pxx_inds,lon_pxx_inds]

files_list = [] 
for file in sorted(glob.glob(dir_fwi+'*.nc')):
...
file_event = dir_event+'CESM2-LE_'+ens_string[nn]+'_allens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_daily_2d_event_numbers_'+str(year0)+'-'+str(year1)+'_U10_NWHemi.nc'
...
nc_event.createVariable('event',fwi_labeled_2d.dtype,('date','lat','lon'))
nc_event.variables['date'][:] = date
nc_event.variables['lat'][:] = lat
nc_event.variables['lon'][:] = lon
nc_event.variables['event'][:] = fwi_labeled_2d
