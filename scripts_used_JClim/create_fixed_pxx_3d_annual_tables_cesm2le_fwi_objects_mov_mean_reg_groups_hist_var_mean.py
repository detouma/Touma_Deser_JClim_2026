...
event_files_list = []
for file in sorted(glob.glob(dir_event+'CESM2-LE_*_mean_mapped_'+hist_var+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_'+reg_groups[gg]+'.nc')):
...
file_event = dir_event+'CESM2-LE_'+ens_no_string[ens]+'_mean_mapped_'+hist_var+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_'+reg_groups[gg]+'.nc'
...
# output event file
event_3d_df_nonan.to_csv(dir_tables+'CESM2-LE_'+ens_no_string[ens]+'_FWI_'+str(n_ens_dist)+'_ens_fixed_p'+str(pxx)+'_3d_events_'+str(year0)+'-'+str(year1)+'_'+reg_groups[gg]+'_historic_'+hist_var+'_mean.txt', header=True, index=None, sep=' ', mode='w', float_format = "%.2f" )
