...
event_files_list = []
for file in sorted(glob.glob(dir_event+'CESM2-LE_*_mean_mapped_'+hist_var+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_'+WUS_regions[rr]+'.nc')):
...
file_event = dir_event+'CESM2-LE_'+ens_no_string[ens]+'_mean_mapped_'+hist_var+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year)+'_p'+str(pxx)+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_'+WUS_regions[rr]+'.nc'
...
# read in FWI
f_fwi = 'FWI_CESM2-LE_'+ens_no_string[ens]+'_historic_'+str(pxx_year)+'_mean_mapped_'+hist_var+'_'+str(year0)+'0101-'+str(year1)+'1231.nc'
...
#   dir_dict[fwi_vars[vv]] = '/glade/scratch/detouma/extreme_objects/'+fwi_vars[vv]+'/'
  dir_dict[fwi_vars[vv]] = '/glade/campaign/cgd/cas/detouma/FWI_vars/'+fwi_vars[vv]+'/' 
  fname_dict[fwi_vars[vv]] = 'CESM2-LE_*_'+str(year0)+'-'+str(year1)+'_members_'+hist_var+'_mapped_to_'+str(pxx_year)+'_mean*'
...
# output event file
event_3d_df_nonan.to_csv(dir_tables+'CESM2-LE_'+ens_no_string[ens]+'_FWI_'+str(n_ens_dist)+'_ens_fixed_p'+str(pxx)+'_3d_events_'+str(year0)+'-'+str(year1)+'_'+WUS_regions[rr]+'_historic_'+hist_var+'_mean.txt', header=True, index=None, sep=' ', mode='w', float_format = "%.2f" )
