...
dir_obj = '/glade/campaign/cgd/cas/detouma/fwi_objects/3d_var_mapped_objects/grp_objects/mean_mapped/'
dir_tables = '/glade/campaign/cgd/cas/detouma/fwi_objects/object_tables/grp_tables/mean_mapped/'
...
var_ind = int(sys.argv[3]) # fixed variable
...
print('CESM2-LE_*_FWI_'+str(n_ens_dist)+'_ens_'+pxx_type+'_p'+pxx_string+'_3d_events_'+str(year0)+'-'+str(year1)+'_*_historic_'+hist_var+'_mean.txt')
for file in sorted(glob.glob(dir_tables+'CESM2-LE_*_FWI_'+str(n_ens_dist)+'_ens_'+pxx_type+'_p'+pxx_string+'_3d_events_'+str(year0)+'-'+str(year1)+'_*_historic_'+hist_var+'_mean.txt')):
...
# read in FWI
f_fwi = 'FWI_CESM2-LE_'+ens_string+'_historic_'+str(pxx_year0)+'_mean_mapped_'+hist_var+'_'+str(year0)+'0101-'+str(year1)+'1231.nc'
...
for vv in range(0,len(fwi_vars),1):
 if (hist_var_select[vv]==0):
  ...
  fname_dict[fwi_vars[vv]] = 'CESM2-LE_*_'+str(year0)+'-'+str(year1)+'_members_'+hist_var+'_mapped_to_'+str(pxx_year0)+'_mean*'
...
object_file = 'CESM2-LE_'+ens_string+'_mean_mapped_'+hist_var+'_to_'+str(n_ens_dist)+'ens_'+str(pxx_year0)+'_p'+pxx_string+'_FWI_event_numbers_'+str(year0)+'-'+str(year1)+'_'+reg_groups[gg]+'.nc'
...
for pp in range(0,nperiods,1):
	...
	events_char_pp_xr.to_netcdf(dir_comps+'CESM2-LE_'+ens_string+'_mean_mapped_'+hist_var+'_p'+pxx_string+'_FWI_events_char_composites_'+period_str[pp]+'_'+pxx_type+'_'+reg_groups[gg]+'_'+season+'.nc')
	print('\n')
